...
namespace MetaBlog.Application.Features.Posts
{
    ...
    {
        Task<PaginatedList<PostDto>> GetPostsAsync(int pageNumber, int pageSize, string? searchTerm, Guid? authorId, DateTime? createdAfter, string? sortBy, bool? sortDescending,Guid?currentUserId, CancellationToken ct);
